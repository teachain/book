...
func GetAllRecords()([]RecordView,error){
	...
	_,err:=o.Raw("select r.*,de.name as device_name, d.name as driver_name from record r join driver d join device de on r.driver_id=d.id and r.device_id=de.id").QueryRows(&records)
	return records,err
}
